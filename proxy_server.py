...
class _Proxy(ThreadedModule):
    ...
    def __init__(self, client_uuid, sock, port_map):
        """
                        ...
                        if event & ~select.EPOLLIN:
                            ...
                            try:
                                ...
                                log.info('Client(%s:%d) ----> Server(%s:%d)' %
                                         (conn2.getpeername()[0], conn2.getpeername()[1],
                                          conn1.getpeername()[0], conn2.getpeername()[1]))
                            except Exception as e:
                                log.error(e)
...
class ProxyServer(ThreadedModule):
    ...
    def __consult_mapping_ports(self, sock):
        """协商需要映射的端口：只有同时满足在客户端已开启对应端口服务，且指定端口在服务端未被占用时才有效。
        ...
        """
        cmd, data = Protocol.request_client(sock, Protocol.Command.QUERY_MAPPING_PORTS,
